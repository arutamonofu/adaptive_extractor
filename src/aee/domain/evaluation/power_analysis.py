...
def load_article_json(filepath: Path) -> dict:
    ...
    if isinstance(data, dict):
        if 'experiments' in data and data['experiments']: return data['experiments'][0]
...
def compute_f1_for_extraction(
    ...
    # Сбор атрибутов GT
    gt_attributes = {f: (str(gt_record[f]) if gt_record.get(f) is not None else None) for f in fields}
...
def bootstrap_sigma_d(d: np.ndarray, n_boot: int = 1000) -> float:
    ...
    samples = [np.std(np.random.choice(d, size=len(d), replace=True), ddof=1) for _ in range(n_boot)]
    return float(np.mean(samples))

def calculate_required_n(sigma_d: float, delta: float, alpha: float, power: float) -> int:
...
def compute_power_curve(n_range: np.ndarray, sigma_d: float, delta: float, alpha: float) -> np.ndarray:
    powers = []
    for n in n_range:
        if n < 2: 
            powers.append(0)
